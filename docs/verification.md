...
验证日期：2026-09-14。主机：macOS ARM64。

- 真实 Electron 集成测试 **23 passed**，新增覆盖：空文档在页面任意位置点击即可编辑、点击位置映射到 Markdown 源码光标、方向键与退格跨段落编辑、关于对话框作者与仓库信息、检查更新（本地模拟发布源）及外部链接白名单。
- 自动更新（macOS 打包应用 `release/mac-arm64/Mardar.app`）：本地模拟发布源提供真实的 v1.1.5 ZIP。校验值被篡改时拒绝安装并提示“安装包 SHA-256 校验失败”；校验通过后应用退出、替换应用包并自动重新启动，替换后版本为 1.1.5。
- Windows 与 Linux 更新路径（NSIS 安装程序、AppImage 原地替换）未在实机验证。
