...
function changed() { metadata(); clearTimeout(timer); timer = setTimeout(() => render().catch(e => status(e.message)), 220); }
...
async function load(doc) { editor.value = doc.content; saved = editor.value; name = doc.name; base = doc.base || ''; format = 'markdown';  changed(); await render(); if ($('.panes').dataset.view === 'live') await renderLive(); await refreshRecent(); }
