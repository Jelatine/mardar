...
test('HTML read write and untrusted content sanitization', async () => {
  const file = path.join(folder, 'page.html');
  await writeFile(file, '<h1>HTML 文档</h1><p><strong>富文本</strong> $E=mc^2$</p><pre class="mermaid">graph LR\nA-->B</pre><script>window.hacked=true</script><img src=x onerror="window.hacked=true">');
  await chooseOpen(file); await page.locator('#open').click();
  await expect(page.locator('#format')).toHaveValue('html');
  await expect(page.locator('#preview h1')).toHaveText('HTML 文档');
  await expect(page.locator('#preview .katex')).toBeVisible();
  await expect(page.locator('#preview .mermaid svg')).toBeVisible();
  expect(await page.evaluate(() => window.hacked)).toBeUndefined();
  await page.locator('#editor').fill('<h1>已编辑 HTML</h1><p>内容</p>');
  const target = path.join(folder, 'copy.html'); await chooseSave(target); await page.locator('#save-as').click();
  await expect.poll(() => readFile(target, 'utf8')).toBe('<h1>已编辑 HTML</h1><p>内容</p>');
});
